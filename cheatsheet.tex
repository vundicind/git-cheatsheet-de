...
\begin{tikzpicture}
  \draw (0,1) node[anchor=north west,font=\Huge\bf\fontfamily{phv}\selectfont]{Breviar de comenzi Git}
  (.1cm,.3cm) -- (28.2cm,.3cm)
  ([xshift=1mm]23.7cm,.7cm)
  node[font=\footnotesize,color=black!85,anchor=north east]{
    \copyright{} 2013 R. Dumbrăveanu
  }
  ++(0,.3cm)
  node[font=\footnotesize,color=black!85,anchor=north east]{
    Versiune de la \today
  ...
  node[font=\footnotesize,color=black!85,anchor=north west]{
    Licența Atribuire-Necomercial-FărăModificări 3.0 România (CC BY-NC-SA 3.0 RO).
  }
  ++(0,.3cm)
  node[font=\footnotesize,color=black!85,anchor=north west]{
    https://github.com/vundicind/git-cheatsheet-ro
  }
  ;
  \draw
  ...
  ++(0,-4.6cm) node[below right,text width=7.5cm,rounded corners,fill=gray!15,inner sep=1ex,drop shadow]
  {
    ...
    \begin{tabular}{ll}
    \texttt{git add} & Marchează fișier\\
...
++(1.4cm,-4.3cm)     node[rounded corners=2pt,fill=gray!25,minimum height=1cm] (w) {Dosarul local}
